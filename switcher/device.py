...
class dxlS500R:
    ...
    def __init__(
        ...
        # Check control mode
        self.pprint( self.ping() ) 
        self.extendedPositionControl = (
            self.get_control_mode == "ExtendedPositionControl"
        )
        self.enable_torque()
    ...
    def one_way_turn_to(self, pos, IN_TICS=False):
        ...
        if newPosition > currentPosition:
            ...
            self.set_control_mode(
                "PositionControl"
            )  # Current position gets translated to [-180,180]
            currentPosition = self.get_current_position(IN_TICS=IN_TICS)

            if IN_TICS:
                if (
                    currentPosition >= (POSITION_LIMIT_32 * 2) + MIN_POS_LIMIT
                ):  # means current position is [MIN_POS_LIMIT, 0)
                    currentPosition -= POSITION_LIMIT_32 * 2
                if newPosition < currentPosition:
                    newPosition += MAX_POS_LIMIT * 2
            else:
                if (
                    currentPosition >= MAX_EXTENDED_POSITION - 180
                ):  # means current position is [-180, 0)
    ...
    def enable_torque(self):
        ...
        dxl_comm_result, dxl_error = self.packetHandler.write1ByteTxRx(
            self.portHandler, self.ID, ADDR_TORQUE_ENABLE, TORQUE_ON
        )
    ...
    def disable_torque(self):
        ...
        dxl_comm_result, dxl_error = self.packetHandler.write1ByteTxRx(
            self.portHandler, self.ID, ADDR_TORQUE_ENABLE, TORQUE_OFF
        )
        self.checkCommStatus(dxl_comm_result, dxl_error)
    ...
    def set_goal_torque(self, goal_torque):
        ...
        dxl_comm_result, dxl_error = self.packetHandler.write2ByteTxRx(
            self.portHandler, self.ID, ADDR_GOAL_TORQUE, goal_torque
        )
        self.checkCommStatus(dxl_comm_result, dxl_error)

    def get_goal_torque(self):
        """set goal torque on motor"""
        dxl_read_value, dxl_comm_result, dxl_error = self.packetHandler.read2ByteTxRx(
            self.portHandler, self.ID, ADDR_GOAL_TORQUE
        )
        self.checkCommStatus(dxl_comm_result, dxl_error)
    ...
    def get_current_velocity(self):
        ...
        dxl_read_value, dxl_comm_result, dxl_error = self.packetHandler.read4ByteTxRx(
            self.portHandler, self.ID, ADDR_PRESENT_VELOCITY
        )
        self.checkCommStatus(dxl_comm_result, dxl_error)
    ...
    def set_velocity(self, deg_per_sec):
        ...
        dxl_comm_result, dxl_error = self.packetHandler.write4ByteTxRx(
            self.portHandler, self.ID, ADDR_GOAL_VELOCITY, tics
        )
        self.checkCommStatus(dxl_comm_result, dxl_error)
    ...
    def get_goal_velocity(self):
        ...
        dxl_read_value, dxl_comm_result, dxl_error = self.packetHandler.read4ByteTxRx(
            self.portHandler, self.ID, ADDR_GOAL_VELOCITY
        )
        self.checkCommStatus(dxl_comm_result, dxl_error)
        return dxl_read_value / VEL_SCALE_FACTOR
    ...
    def get_current_position(self, IN_TICS=False):
        ...
        dxl_read_value, dxl_comm_result, dxl_error = self.packetHandler.read4ByteTxRx(
            self.portHandler, self.ID, ADDR_PRESENT_POSITION
        )
        self.checkCommStatus(dxl_comm_result, dxl_error)
        if IN_TICS:
            return dxl_read_value
    ...
    def set_position(self, pos, IN_TICS=False):
        ...
        dxl_comm_result, dxl_error = self.packetHandler.write4ByteTxRx(
            self.portHandler, self.ID, ADDR_GOAL_POSITION, tics
        )
        self.checkCommStatus(dxl_comm_result, dxl_error)
    ...
    def is_moving(self):
        ...
        dxl_read_value, dxl_comm_result, dxl_error = self.packetHandler.read1ByteTxRx(
            self.portHandler, self.ID, ADDR_MOVING
        )
        self.checkCommStatus(dxl_comm_result, dxl_error)
        return dxl_read_value
    ...
    def get_control_mode(self):
        ...
        dxl_read_value, dxl_comm_result, dxl_error = self.packetHandler.read1ByteTxRx(
            self.portHandler, self.ID, ADDR_OP_MODE
        )
        self.checkCommStatus(dxl_comm_result, dxl_error)
        if dxl_read_value == 3:
        ...
        return list(OPERATING_MODES.keys())[
            list(OPERATING_MODES.values()).index(int(dxl_read_value))
        ]
    ...
    def set_control_mode(self, mode):
        ...
        if mode not in OPERATING_MODES.keys():
            raise dxlS500R_Exception(
                f"{mode} is not a valid control mode\nOptions: {OPERATING_MODES.keys()}"
            )
        self.disable_torque()  # Writing to ROM not permitted without disabling torque
        dxl_comm_result, dxl_error = self.packetHandler.write1ByteTxRx(
            self.portHandler, self.ID, ADDR_OP_MODE, OPERATING_MODES[mode]
        )
        self.checkCommStatus(dxl_comm_result, dxl_error)
    ...
    def get_id(self):
        ...
        dxl_read_value, dxl_comm_result, dxl_error = self.packetHandler.read1ByteTxRx(
            self.portHandler, self.ID, ADDR_ID
        )
        self.checkCommStatus(dxl_comm_result, dxl_error)
        return dxl_read_value
    ...
    def set_id(self, newID):
        ...
        if not (0 <= newID <= 254):
            raise dxlS500R_Exception(
                f"{newID} is an invalid value. newID must be = [0, 253]"
            )
        dxl_comm_result, dxl_error = self.packetHandler.write1ByteTxRx(
            self.portHandler, self.ID, ADDR_ID, newID
        )
        self.checkCommStatus(dxl_comm_result, dxl_error)
        self.ID = newID
